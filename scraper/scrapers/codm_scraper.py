"""
CODM (Call of Duty: Mobile) scraper for VaultDrop.

Scrapes Reddit RSS feeds for r/CallOfDutyMobile and r/CODMobileLeaks,
...
# Reddit RSS feed URLs for CODM
CODM_RSS_FEEDS = [
    "https://www.reddit.com/r/CallOfDutyMobile/new/.rss",
    "https://www.reddit.com/r/CODMobileLeaks/new/.rss",
]
...
CATEGORY_KEYWORDS: dict[str, list[str]] = {
    "leak": [
        ...
        "unreleased", "season", "update", "patch", "battle pass",
    ],
    "skin": [
    ...
    "weapon": [
        ...
        "pistol", "melee", "ak-47", "m4", "dragunov",
    ],
    "map": [
        "map", "location", "battlefield", "terrain",
    ],
    "event": [
    ...
    "esports": [
        "esports", "tournament", "competitive", "pro", "world championship",
        "masters", "qualifier",
    ],
    "rumor": [
...
class CODMScraper(BaseScraper):
    """Scrapes CODM Reddit RSS feeds for leak / news content."""
    ...
    def scrape(self) -> list[dict]:
        """Run the CODM scraper and return a list of leak items."""
        items: list[dict] = []
        for feed_url in CODM_RSS_FEEDS:
            logger.info("Fetching CODM RSS feed: %s", feed_url)
            entries = self._fetch_reddit_rss(feed_url)
            for entry in entries:
                item = self._entry_to_leak_item(entry)
                if item:
                    items.append(item)
        logger.info("CODM scraper found %d items", len(items))
        return items
    ...
    def _entry_to_leak_item(self, entry: dict) -> dict | None:
        ...
        return {
            ...
            "ai_caption": caption,
            "description": "",
            "severity": "high" if category in ("leak", "rumor") else "normal",
            "is_verified": False,
