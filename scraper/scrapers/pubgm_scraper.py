"""
PUBG Mobile scraper for VaultDrop.

Scrapes Reddit RSS feeds for r/PUBGMobile, r/PUBGMobileLeaks, and r/BGMI,
...
# Reddit RSS feed URLs for PUBGM
PUBGM_RSS_FEEDS = [
    "https://www.reddit.com/r/PUBGMobile/new/.rss",
    "https://www.reddit.com/r/PUBGMobileLeaks/new/.rss",
    "https://www.reddit.com/r/BGMI/new/.rss",
]
...
CATEGORY_KEYWORDS: dict[str, list[str]] = {
    "leak": [
        ...
        "unreleased", "season", "update", "patch", "royale pass",
    ],
    "skin": [
        "skin", "bundle", "cosmetic", "outfit", "camo", "camouflage",
        "upgrade", "ornament",
    ],
    "weapon": [
        "weapon", "gun", "ar", "smg", "sniper", "shotgun", "lmg",
        "pistol", "melee", "m416", "akm", "awm", "kar98k",
    ],
    "map": [
        "map", "location", "erangel", "miramar", "sanhok",
        "vikendi", "livik", "nusa",
    ],
    "event": [
        "event", "limited", "challenge", "reward", "seasonal",
        "holiday", "anniversary", "collab", "collaboration",
    ],
    "esports": [
        "esports", "tournament", "competitive", "pro", "pmpl",
        "pmgc", "global championship", "qualifier",
    ],
    "rumor": [
...
class PUBGMScraper(BaseScraper):
    ...
    def scrape(self) -> list[dict]:
        """Run the PUBGM scraper and return a list of leak items."""
        items: list[dict] = []
        for feed_url in PUBGM_RSS_FEEDS:
            logger.info("Fetching PUBGM RSS feed: %s", feed_url)
            entries = self._fetch_reddit_rss(feed_url)
            for entry in entries:
                item = self._entry_to_leak_item(entry)
                if item:
                    items.append(item)
        logger.info("PUBGM scraper found %d items", len(items))
        return items
